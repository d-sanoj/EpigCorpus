...
1,424,314 (abbreviation, expansion) pairs from 588,509 records.

Building the dataset required auditing the source, and that audit is the larger
result. A complete character census of 39,470,885 characters shows that EDCS
does not preserve the numeral vinculum: U+0305 occurs 2 times, U+0304
...
marked with an overline; the construction occurs 1,875 times,
65.4% of them after the sestertius sign. A single ASCII `|` carries at
least eight unrelated epigraphic glyphs across 376 distinct forms.
...
retained material by 11.1× and 16.3× a bootstrap null in
...
for `V` is 0.3709, about 12.2× an inscription-level permutation
null. The predictive value does not. Province conditioning gains +0.0833
for a lookup model on provinces seen in training, +0.0000 on provinces
withheld from it, and -0.0070 and -0.0070 for a feature-based
...
project could fund. From 588,509 records we extract 1,424,314 pairs.
...
1. **A dataset.** 1,424,314 pairs over 588,509 records, with
...
1,424,314 derived labels, trading label reliability for scale, and Section 6
...
ambition. 54.5% of ambiguity among evaluable keys is lexical, involving
...
form. The snapshot used here comprises 588,509 records and 39,470,885
characters, pinned by SHA-256 (`9ebea1a7a5742d055af3b7059703cd8fd1ea708578c3ea43b9882f5873242317`) because the database is live
and grows. A re-harvest reproduces none of the figures below.

The records are not 588,509 inscriptions, and the difference matters for
construction. Record identifiers take the form `EDCS-<8 digits>-<n>`, where the
trailing segment marks a face or panel of one monument. 542,854 distinct
monuments underlie the 588,509 records, and 45,655 records share a
...
We ran a complete character census over all 588,509 records, covering
...
The encoding pipeline is not the cause. 208 instances of U+0323, the
...
convention. At 208 instances, though, no Leiden diacritic is
...
parentheses. The construction `N(milia)` occurs 1,875 times, and
65.4% of those occurrences follow `HS`, the sestertius sign, with small
...
The effect on dataset construction is direct. 2,338 such pairs sit in the
...
We enumerate every distinct `|(...)` form in the corpus: 376 forms
across 16,194 occurrences. Grouping them by the sign they plausibly stand
...
rule finds 8,986, a 41% undercount. The corrected forms then agree with an
...
excluded for ten reasons, leaving no residual. At record level, 380,282
...
implementation's code. The two agree on 1,424,238 pairs, 99.995% of the
...
forms it judges. 8,986 pairs are corrected and 3,350 are left
...
after 1000 AD individually, 19 of 21 prove correctly dated:
...
Three provinces, Britannia, Mauretania Caesariensis, Pannonia inferior, are withheld entirely for a second split. We
...
Monument grouping does not prevent all leakage. 18,381 test rows
...
material differs from the retained material by province TVD 0.131
against a bootstrap null p95 of 0.012 (11.1×) and century TVD
0.097 against 0.006 (16.3×). Dropped texts average
504 characters against 280 for retained ones.
...
63.25% of such tokens have a wholly editorial abbreviation, where the
editor inferred the letters and then the expansion of the letters they inferred,
and only 1.35% are fully attested. Recovering this material into a
...
(0.96%) carry an expansion differing from another row that presents the
...
Separately, 66.4% of distinct expansion forms also occur in the corpus
...
**M1**, a most-frequent-expansion lookup (57,054 parameters), is the
floor and trivially reproducible. **M2** is a logistic-regression candidate
re-ranker (1,048,580 parameters) over character n-grams of the abbreviation,
...
encoder, `bowphs/LaBerta` (137,180,135 parameters), with a mean-pooled
...
100,000 rows, 8.8% of the training split, at a single seed, for
...
The hypothesis fails. Province and century gain +0.0833 for a model that
...
between province and expansion choice for `V` is 0.3709, about
12.2× a permutation null computed at the level of whole inscriptions
...
parameters and training time. M3 was trained on 8.8% of the data
...
2. The task is conditioned on the editor having judged an abbreviation present.
   66.4% of expansion forms also occur uncontracted.
3. Editorial disagreement is at least 0.96%, and that figure is a
   floor.
4. The exclusion filter is bias-inducing, at 11.1× and
   16.3× a null in province and century.
5. EDCS assigns a resolvable single-century date to 36.0% of records,
...
The benchmark exists: 1,424,314 labelled pairs over 588,509 records, with
...
command, which verifies the corpus SHA-256 (`9ebea1a7a5742d055af3b7059703cd8fd1ea708578c3ea43b9882f5873242317`) before running
...
would overturn them, in 41 entries.
