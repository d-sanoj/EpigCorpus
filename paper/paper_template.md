...
`D(is) M(anibus)`. We mine those parentheses and obtain **{{pairs}}**
(abbreviation, expansion) pairs from **{{records}}** records.
...
asserted. A complete character census of **{{chars}}** characters establishes
that the numeral vinculum is **not preserved**: U+0305 occurs {{u0305}} times
...
occurs {{milia_occ}} times, {{hs_pct}}% of them preceded by the sestertius sign.
A single ASCII `|` stands for at least eight unrelated epigraphic glyphs across
**{{pipe_forms}} distinct forms**. Plural-marking geminatio doubles an
...
merely filtering it, differing from the retained material at {{prov_ratio}}×
and {{cent_ratio}}× a bootstrap null in province and century respectively.
...
between province and expansion for `V` is {{nmi_v}}, some {{nmi_ratio}}× an
inscription-level permutation null. The predictive value is not. Province
conditioning is worth **{{m1_delta}}** to a lookup model on provinces seen in
training, **exactly {{m1h_delta}}** on provinces withheld from it, and nothing
at all — {{m2_delta}} and {{m3_delta}} — to a feature-based re-ranker and a
...
{{records}} records we extract {{pairs}} such pairs.
...
1. **A dataset.** {{pairs}} (abbreviation, expansion) pairs over {{records}}
...
inscriptions against {{pairs}} derived labels — manual precision against derived
...
fills. Our results, however, also qualify the ambition: **{{lex_pct}}%** of
...
form. The snapshot used here comprises **{{records}}** records and
**{{chars}}** characters, pinned by SHA-256 (`{{corpus_hash}}`) because the
database is live and grows; a re-harvest reproduces none of the figures below.

The records are **not** {{records}} inscriptions, and the distinction matters
...
**{{monuments}}** distinct monuments underlie the {{records}} records, with
{{multiseg}} records sharing a base identifier with at least one other. All
...
We ran a complete character census over all {{records}} records — all
...
This is not a limitation of the encoding pipeline. {{underdots}} instances of
...
convention. The honest qualifier is that at {{underdots}} instances no Leiden
...
{{milia_occ}} times, and **{{hs_pct}}% of those occurrences are immediately
...
The consequence for dataset construction is direct. **{{fused}}** such pairs sit
...
We enumerate every distinct `|(...)` form in the corpus: **{{pipe_forms}}
forms, {{pipe_occ}} occurrences**. Grouping them by the sign they plausibly
...
implementation found 5,293 cases; the trailing-run rule finds **{{gem}}**, a 41%
...
excluded for ten reasons, with zero residual. At the record level, {{recs_paren}}
...
implementation's code**. The two agree on **{{agree}}** pairs, {{agree_pct}}% of
...
forms it judges. {{gem}} pairs are corrected; {{gem_unresolved}} are left
...
records dated after 1000 AD individually, {{late_ok}} of {{late_total}} prove to
...
Three provinces — {{heldout_list}} — are withheld entirely for a second split,
...
{{dup_rows}} test rows ({{dup_pct}}%) are byte-identical to a training row,
...
{{prov_tvd}}** against a bootstrap null p95 of {{prov_null}}
(**{{prov_ratio}}×**) and **century TVD {{cent_tvd}}** against {{cent_null}}
(**{{cent_ratio}}×**). Dropped texts average {{drop_len}} characters against
{{kept_len}} for retained ones.
...
**{{circ_pct}}%** of such tokens have a wholly editorial abbreviation — the
editor inferred the letters *and* the expansion of the letters they inferred —
and only **{{attested_pct}}%** are fully attested. Any recovery of this material
...
({{noise_pct}}%)** carry an expansion that differs from another row presenting
...
Separately, **{{plain_pct}}% of distinct expansion forms also occur in the
...
- **M1**, a most-frequent-expansion lookup ({{m1_params}} parameters). The
  floor, and trivially reproducible.
- **M2**, a logistic-regression candidate re-ranker ({{m2_params}} parameters)
  ...
  row.
- **M3**, a fine-tuned Latin encoder, `bowphs/LaBerta` ({{m3_params}}
...
**{{m3_rows}}** rows — {{m3_pct}}% of the training split — at a single seed,
...
**The hypothesis fails.** Province and century are worth {{m1_delta}} to a model
...
choice for `V` is {{nmi_v}}, some {{nmi_ratio}}× a permutation null computed at
...
stated plainly: M3 was trained on {{m3_pct}}% of the data available to M2, so
...
2. **The task is conditioned on the editor having judged an abbreviation
   present** ({{plain_pct}}% of expansion forms also occur uncontracted).
3. **Editorial disagreement is ≥{{noise_pct}}%**, and that figure is a floor.
4. **The exclusion filter is bias-inducing** ({{prov_ratio}}× and
   {{cent_ratio}}× a null in province and century).
5. **EDCS assigns a resolvable single-century date to only {{dated_pct}}% of
...
7. **{{dup_pct}}% of the primary test split is byte-identical to a training
...
{{pairs}} labelled pairs over {{records}} records, with frozen monument-grouped
...
a single command, which verifies the corpus SHA-256 (`{{corpus_hash}}`) before
...
and what would overturn them, in {{n_decisions}} entries.
